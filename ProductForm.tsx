...
const ProductForm: React.FC<ProductFormProps> = ({ editingProduct, onSubmit, initialProduct, onCancel }) => {
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
              <Link to="/" className="text-sm font-medium text-stone-600 hover:text-brand-600 transition">
              <span>Home</span>
              </Link>

              <button
              className={`text-sm font-medium text-stone-600 hover:text-brand-600 transition ${isButtonDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-md p-8 flex flex-col gap-6 max-w-xl mx-auto">
        <button className="w-fit px-3 py-1.5 bg-stone-200 rounded-md" onClick={handleNavigateBack}>Voltar</button>
        <h2 className="text-xl font-semibold text-center">{editingProduct ? 'Editar Produto' : 'Cadastrar Produto'}</h2>
        ...
        <div className="flex flex-col gap-2">
          <label className="font-medium text-stone-700">Benefícios (separados por vírgula)</label>
          <input
            type="text"
            className="border border-stone-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500"
