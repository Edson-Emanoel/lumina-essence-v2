// Função para montar o payload Pix no padrão EMV
function generatePixPayload({ chave, valor, nome, cidade }: { chave: string; valor: string; nome: string; cidade: string }) {
  // Helper para montar campo EMV
...
export default function PixCheckout({ cart, total, onClose, onSuccess }: PixCheckoutProps) {
  ...
  const handleGeneratePix = async (e: React.FormEvent) => {
    ...
    const nome = 'Lumina Essence';
    const cidade = 'Sao Paulo';
    const payloadPix = generatePixPayload({ chave: pixKey, valor: valorFormatado, nome, cidade });
    const mockPixData: PixData = {
      qrCodeBase64: `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(payloadPix)}`,
      copyPasteKey: payloadPix,
